...
<div class="uploader clearfix">
    <input class="file" ref="avatarInput" id="fileUp" type="file" accept="image/*" multiple="false" @change="add"/>
    <div id="fileList" class="uploader-list">
        <div class="file-item" v-for="(item,index) in fileUrls" :key="index">
        <img v-if="item.loaded" :src="item.url">
  ...
  methods: {
    ...
    update(param){
        ...
        headers:{'Content-Type':'multipart/form-data'}
          ...
          vm.fileUrls.push({url:data.data[0].wwwurl,loaded:true});
          THAT.$refs.avatarInput.value = "";
          vm.$emit('get-imgurl',vm.fileUrls);
        });
    },
    // 删除图片
    delite(index){
      vm.fileUrls.splice(index,1);
      vm.$emit('get-imgurl',vm.fileUrls);
    },
    clearUrls(){
      vm.fileUrls = []  
